...
The leader keeps serving. While the follower is unreachable it replicates to S3 instead, which adds S3 latency to the write path until the follower returns and catches up. If the **leader** goes down, the follower takes over once its heartbeat lease expires (`--heartbeat-lease-duration-ms`, 1.5 s by default), then CAS's the already-expired S3 lease: writes pause for around 1.3 s in practice, while reads from the survivor continue throughout. The 30 s `--s3-lease-duration-ms` is the cold-case backstop, not the steady-state failover time. See [Leader election and S3 leases](/operations/leader-election-s3) for the timing and [Upgrading](/operations/upgrading) for the rolling-restart procedure.
