...
Identity is distinct from the TLS handshake: TLS authenticates the connection, identify establishes who the client is for idempotency and access control. See [Clients and identity](/concepts/identity). Failures return the `Identify*` codes in the [error reference](/reference/error-codes).
