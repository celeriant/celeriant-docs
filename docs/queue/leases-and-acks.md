...
- `Nack` does not bump on its own. The next fresh Lease does.
...
When `delivery_count > max_delivery_attempts`, the next consume routes the message per the queue's [dlq_strategy](/queue/dlq-and-poison).
